...
export class PhonesPageService {
    ...
    getAllPhonesSorted(keyWordText, sortValue) {
        let result = this.searchService(keyWordText);
        result = this.sortService(result, sortValue);
    ...
    searchService(keyWordText) {

        let searchedPhones = phones.filter(function (phonesItem) {

            return phonesItem.name.toLowerCase().includes(keyWordText.toLowerCase());
        });
        // console.dir(searchedPhones);
        // console.log('Из PhonesPageService: ' + keyWordText);

        if (searchedPhones.length > 0) {
    ...
    sortService(phones, sortValue) {
        ...
        let sortedPhones = phones.sort(
            function (stringFirst, stringSecond) {
                if (stringFirst[field] > stringSecond[field]) return 1;
                if (stringFirst[field] < stringSecond[field]) return -1;
            }
        )
